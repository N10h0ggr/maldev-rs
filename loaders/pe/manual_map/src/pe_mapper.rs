...
/// Adjusts memory protections for all mapped sections based on their characteristics.
///
/// All sections are initially mapped as RW. This function applies the final
/// permissions (R/W/X) as expected by the PE.
pub fn fix_memory_permissions(image: &PeImage, base: *mut u8) -> Result<(), PeError> {
    let section_count = image.nt_headers().file_header().NumberOfSections as usize;
    debug!("protect: fixing permissions for {} sections", section_count);
    ...
    info!("protect: memory permissions applied");
    Ok(())
}
